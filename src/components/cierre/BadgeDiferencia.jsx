...
export default function BadgeDiferencia({ diferencia }) {
  ...
  if (abs === 0)
    return <span className="text-xs font-black text-green-600 bg-green-50 px-2 py-0.5 rounded-full">Sin diferencia</span>;
  if (abs <= 500)
    return <span className="text-xs font-black text-yellow-600 bg-yellow-50 px-2 py-0.5 rounded-full">{clp(diferencia)}</span>;
